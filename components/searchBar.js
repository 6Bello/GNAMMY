...
const SearchBar = ({ loadingTrue, loadingFalse, updateRecipes, updateUsersSearched }) => {
  ...
  const searchRecipeByName = () => {    
    ...
    loadingTrue();
    axios
      .get(`http://gnammy.mywire.org:443/getRecipesByName/${searchText}`) // Effettua una richiesta GET all'API specificata
      .then((response) => {
  ...
  const searchUsersByName = () => {
    ...
    loadingTrue();
    axios
    .get(`http://gnammy.mywire.org:443/getUsers/${searchText}`)
    .then((response) => {
